...
class ClubSectionTeamConfigAdvancedFilterPage {
    /**
     * @param {Page} page
     */
    constructor(page) {
        ...
        // Team Configuration tab on section detail page
    ...
    async openTeamConfigFilter() {
        const isOpen = await this.filterDialog.isVisible().catch(() => false);
        if (isOpen) {
            console.log('Team Configuration filter dialog already open – skipping open click');
            return;
        }

        // Strategy 1: filter icon near Team Configuration heading
        ...
        if (s1Visible) {
            ...
            console.log('✓ Opened Team Configuration filter dialog (near heading)');
            return;
        }
        ...
        await this.filterDialog.waitFor({ state: 'visible', timeout: 10000 });
        console.log('✓ Opened Team Configuration filter dialog (first visible filter icon)');
    }
    ...
    startCapturingAPI() {
        ...
        console.log('✓ Listening for Team Configuration advanced filter API response');
    }
    ...
    mapFieldLabel(fieldLabel) {
        const mapping = {
            ...
            'Team Classification': 'classification.name',
            'Tags':                'tags',
...
module.exports = ClubSectionTeamConfigAdvancedFilterPage;
