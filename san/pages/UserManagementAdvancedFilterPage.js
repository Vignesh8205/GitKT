...
class UserManagementAdvancedFilterPage {
    /**
     * @param {Page} page
     */
    constructor(page) {
    ...
    async clickSettingsTab() {
        const tab = this.page.locator('[id="Settings"]').first();
    ...
    async clickUserManagementTab() {
        const tab = this.page.locator('span.e-list-text').filter({ hasText: /^User Management$/ }).first();
        await tab.waitFor({ state: 'visible', timeout: 60000 });
        await tab.scrollIntoViewIfNeeded();
        await tab.click();
        await this.page.waitForLoadState('networkidle', { timeout: 15000 }).catch(() => {});
        console.log('✓ Clicked User Management tab');
    }
    ...
    async verifyUserManagementPageTitle() {
        await this.page.waitForLoadState('networkidle', { timeout: 15000 }).catch(() => {});
        const title = this.page.locator(
            "//div[normalize-space(text())='User Management'] | " +
            "//h1[normalize-space(text())='User Management'] | " +
            "//h2[normalize-space(text())='User Management'] | " +
            "//span[normalize-space(text())='User Management'] | " +
            "//span[contains(@class,'text-') and normalize-space()='User Management']"
        );
        await title.first().waitFor({ state: 'visible', timeout: 20000 });
        console.log('✓ User Management page title verified');
    }
    ...
    startCapturingAPIResponse() {
        ...
        console.log('✓ Listening for User Management list API response');
    }
    ...
    async awaitAPIResponse() {
        ...
        console.log(`✓ Captured User Management API URL: ${response.url()}`);
    ...
    async verifyAPITotalGreaterThanZero(apiBody) {
        ...
        if (count === 0 || items.length === 0) {
            console.log('ℹ No records found in User Management for the applied filter — grid displays "No records found".');
        } else {
            console.log(`✓ User Management API total count is ${count}, items in page: ${items.length}`);
        }
    }
    ...
    printAPIResponseJSON(apiBody) {
        console.log('──────── USER MANAGEMENT ADVANCED FILTER API RESPONSE (JSON) ────────');
...
module.exports = UserManagementAdvancedFilterPage;
