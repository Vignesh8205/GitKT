...
class CompetitionConfigAdvancedFilterPage {
    /**
     * @param {Page} page
     */
    constructor(page) {
    ...
    async verifyPageTitle() {
        ...
        const title = this.page.locator(
            "//div[normalize-space(text())='Competition Configuration'] | " +
            "//h1[normalize-space(text())='Competition Configuration'] | " +
            "//h2[normalize-space(text())='Competition Configuration'] | " +
            "//span[normalize-space(text())='Competition Configuration']"
        );
        await title.first().waitFor({ state: 'visible', timeout: 20000 });
        const actualTitle = (await title.first().textContent())?.trim();
        expect(actualTitle).toBe('Competition Configuration');
        console.log('✓ Competition Configuration page title verified');
    }
    ...
    startCapturingAPIResponse() {
        this._apiResponsePromise = this.page.waitForResponse(
            (r) =>
                ...
                (r.url().toLowerCase().includes('config') || r.url().toLowerCase().includes('configuration')) &&
                r.url().toLowerCase().includes('list') &&
                r.request().method() === 'POST' &&
                r.status() === 200,
            { timeout: 90000 }
        );
        console.log('✓ Listening for Competition Configuration list API response');
    }
    ...
    async awaitAPIResponse() {
        ...
        console.log(`✓ Captured Competition Configuration API URL: ${response.url()}`);
    ...
    async verifyAPITotalGreaterThanZero(apiBody) {
        ...
        if (count === 0 || items.length === 0) {
            console.log('ℹ No records found in Competition Configuration for the applied filter — grid displays "No records found".');
        } else {
            console.log(`✓ Competition Configuration API total count is ${count}, items in page: ${items.length}`);
        }
    }
    ...
    printAPIResponseJSON(apiBody) {
        console.log('──────── COMPETITION CONFIG ADVANCED FILTER API RESPONSE (JSON) ────────');
...
module.exports = CompetitionConfigAdvancedFilterPage;
