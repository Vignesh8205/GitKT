...
class ClubSectionNotesPage {
    constructor(page) {
        ...
        this.sectionsTab = page.locator("//div[contains(text(),'Sections ')]");
        this.sectionLink = (sectionName) => page.locator(`//td//div[contains(text(),'${sectionName}')] `).first();
    ...
    async clickSectionsTab() {
        await this.sectionsTab.waitFor({ state: 'visible', timeout: 10000 });
        await this.sectionsTab.click();
    }

    async clickSection(sectionName) {
        const section = this.sectionLink(sectionName);
    ...
    async enterClubSectionNoteContent(noteContent) {
...
module.exports = ClubSectionNotesPage;
