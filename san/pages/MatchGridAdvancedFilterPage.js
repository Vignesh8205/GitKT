...
class MatchGridAdvancedFilterPage {
    /**
     * @param {Page} page
     */
    constructor(page) {
    ...
    async navigateToMatchGridTab() {
        await this.page.waitForLoadState('networkidle', { timeout: 15000 }).catch(() => {});
        const matchGridTab = this.page.locator(
            "//div[@role='tab' and normalize-space()='Match Grid']"
        );
        await matchGridTab.waitFor({ state: 'visible', timeout: 15000 });
        await matchGridTab.click();
        await this.page.waitForLoadState('networkidle', { timeout: 15000 }).catch(() => {});
    ...
    startCapturingAPIResponse() {
        this._apiResponsePromise = this.page.waitForResponse(
            (r) =>
                r.url().toLowerCase().includes('match-grids') &&
                r.url().toLowerCase().includes('list') &&
                r.request().method() === 'POST' &&
                r.status() === 200,
            { timeout: 90000 }
        );
        console.log('✓ Listening for Match Grid list API response');
    }
    ...
    async awaitAPIResponse() {
        ...
        console.log(`✓ Captured Match Grid API URL: ${response.url()}`);
    ...
    async verifyAPITotalGreaterThanZero(apiBody) {
        ...
        if (count === 0 || items.length === 0) {
            console.log('ℹ No records found in the Match Grid for the applied filter — grid displays "No records found".');
        } else {
            console.log(`✓ Match Grid API total count is ${count}, items in page: ${items.length}`);
        }
    }
    ...
    printAPIResponseJSON(apiBody) {
        console.log('──────── MATCH GRID ADVANCED FILTER API RESPONSE (JSON) ────────');
    ...
    // ─────────────────────────────────────────────────────────────────
    // API VALIDATION – SINGLE CONDITION (all records must match)
    // ─────────────────────────────────────────────────────────────────
...
module.exports = MatchGridAdvancedFilterPage;
