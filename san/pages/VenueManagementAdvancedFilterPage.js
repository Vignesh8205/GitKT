...
class VenueManagementAdvancedFilterPage {
    /**
     * @param {Page} page
     */
    constructor(page) {
    ...
    async verifyVenueManagementPageTitle() {
        await this.page.waitForLoadState('networkidle', { timeout: 15000 }).catch(() => {});
        const title = this.page.locator(
            "//div[normalize-space(text())='Venue Management'] | " +
            "//h1[normalize-space(text())='Venue Management'] | " +
            "//h2[normalize-space(text())='Venue Management'] | " +
            "//span[normalize-space(text())='Venue Management'] | " +
            "//span[contains(@class,'text-') and normalize-space()='Venue Management']"
        );
        await title.first().waitFor({ state: 'visible', timeout: 20000 });
        console.log('✓ Venue Management page title verified');
    }
    ...
    startCapturingAPIResponse() {
        this._apiResponsePromise = this.page.waitForResponse(
            (r) =>
                r.url().toLowerCase().includes('venue') &&
                r.request().method() === 'POST' &&
                r.status() === 200,
            { timeout: 90000 }
        );
        console.log('✓ Listening for Venue Management API response');
    }
    ...
    async awaitAPIResponse() {
        ...
        console.log(`✓ Captured Venue Management API URL: ${response.url()}`);
    ...
    async verifyAPITotalGreaterThanZero(apiBody) {
        ...
        if (count === 0 || items.length === 0) {
            console.log('ℹ No records found in Venue Management for the applied filter — grid displays "No records found".');
        } else {
            console.log(`✓ Venue Management API total count is ${count}, items in page: ${items.length}`);
        }
    }
    ...
    printAPIResponseJSON(apiBody) {
        console.log('──────── VENUE MANAGEMENT ADVANCED FILTER API RESPONSE (JSON) ────────');
    ...
    mapFieldLabel(fieldLabel) {
        const mapping = {
            ...
            'Venue Name':   'name',
            'Capacity':     'capacity',
            'Surface Type': 'surfaceType.libraryValue',
            'Venue Type':   'venueType.libraryValue',
...
module.exports = VenueManagementAdvancedFilterPage;
