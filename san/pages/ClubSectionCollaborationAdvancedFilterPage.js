...
class ClubSectionCollaborationAdvancedFilterPage {
    /**
     * @param {Page} page
     */
    constructor(page) {
        ...
        // Collaborations tab on section detail page
        this.collaborationsTab = page.locator("//div[text()=' Collaborations ']").first();
    ...
    async navigateToCollaborationsTab() {
        await this.collaborationsTab.waitFor({ state: 'visible', timeout: 15000 });
        await this.collaborationsTab.click();
        await this.page.waitForLoadState('networkidle', { timeout: 15000 }).catch(() => {});
        await this.page.waitForTimeout(1000);
        console.log('✓ Navigated to Collaborations tab on section detail');
    }
    ...
    async openCollaborationsFilter() {
        const isOpen = await this.filterDialog.isVisible().catch(() => false);
        if (isOpen) {
            console.log('Collaborations filter dialog already open – skipping open click');
            return;
        }

        // Strategy 1: filter icon near Collaborations heading
        ...
        if (s1Visible) {
            ...
            console.log('✓ Opened Collaborations filter dialog (near heading)');
            return;
        }
        ...
        await this.filterDialog.waitFor({ state: 'visible', timeout: 10000 });
        console.log('✓ Opened Collaborations filter dialog (first visible filter icon)');
    }
    ...
    startCapturingAPI() {
        ...
        console.log('✓ Listening for Collaborations advanced filter API response');
    }
...
module.exports = ClubSectionCollaborationAdvancedFilterPage;
