...
class CompetitionRankingPositionBasicFilterPage {
    /**
     * @param {Page} page
     */
    constructor(page) {
        ...
        // Ranking Position sub tab on the Ranking page
        this.rankingPositionTab = page.locator(
            "//div[contains(@class,'e-tab-text')]//div[contains(normalize-space(),'Ranking Position')]"
        ).first();

        // Filter icon button on the Ranking Position grid toolbar
        this.rankingPositionFilterBtn = page.locator(
            "button:has(span[style*='filter.svg']), span[style*='filter.svg']"
        ).first();

        // API response capture
        this._rankingPositionResponsePromise = null;
    ...
    async navigateToRankingPositionTab() {
        await this.rankingPositionTab.waitFor({ state: 'visible', timeout: 15000 });
        await this.rankingPositionTab.click();
        await this.page.waitForLoadState('networkidle', { timeout: 15000 }).catch(() => {});
        await this.page.waitForTimeout(1000);
        console.log('✓ Navigated to Ranking Position sub tab');
    }
    ...
    async openFilterDialog() {
        ...
        if (isOpen) {
            console.log('Ranking Position filter dialog already open – skipping click');
            return;
        }

        // Strategy 1: filter icon near Ranking Position grid heading
        const nearPositionBtn = this.page.locator(
            "(//div[contains(normalize-space(),'Ranking Position')]" +
            "/following::button[.//span[contains(@style,'filter.svg')]] | " +
            "//div[contains(normalize-space(),'Ranking Position')]" +
            "/following::span[contains(@style,'filter.svg')])[1]"
        );
        if (await nearPositionBtn.isVisible().catch(() => false)) {
            await nearPositionBtn.click();
            await this.filterDialog.waitFor({ state: 'visible', timeout: 10000 });
            console.log('✓ Opened Ranking Position filter dialog (near Ranking Position heading)');
            return;
        }

        // Strategy 2: any visible filter icon on page
        await this.rankingPositionFilterBtn.waitFor({ state: 'visible', timeout: 10000 });
        await this.rankingPositionFilterBtn.click();
        await this.filterDialog.waitFor({ state: 'visible', timeout: 10000 });
        console.log('✓ Opened Ranking Position filter dialog (first visible filter icon)');
    }
    ...
    async clickApply() {
        ...
        console.log('✓ Clicked Apply button in Ranking Position filter dialog');
    }
    ...
    async clickReset() {
        ...
        console.log('✓ Clicked Reset button in Ranking Position filter dialog');
    }
        ...
        for (const [field, value] of Object.entries(criteria)) {
            ...
            switch (field) {
                ...
                case 'Group Name':
                    ...
                    break;
                case 'Total Teams':
                    await this._fillAutoComplete('Search by Total Teams', value);
                    break;
                case 'Groups':
                    await this._fillAutoComplete('Search by Groups', value);
                    break;
                default:
                    console.warn(`⚠ Unknown Ranking Position filter field: "${field}" – skipping`);
            }
            await this.page.waitForTimeout(300);
        }
    }
    ...
    async _fillAutoComplete(placeholder, value) {
        ...
        const targetInput = await byPlaceholder.count() > 0
            ? byPlaceholder
    ...
    /**
     * Register a response listener for the Ranking Position list API.
     * Must be called BEFORE clicking Apply so the promise is ready.
     */
    startCapturingRankingPositionFilterAPI() {
        ...
        this._rankingPositionResponsePromise = this.page.waitForResponse(
            (response) => {
                const url = response.url().toLowerCase();
                const status = response.status();
                return (url.includes('ranking') || url.includes('position')) &&
                    status >= 200 && status < 400;
            },
            { timeout: 90000 }
        );
        console.log('✓ Started capturing Ranking Position filter API response');
    }

    /**
     * Await the captured Ranking Position API response, print the full JSON, and return the body.
     */
    async awaitAndPrintRankingPositionFilterAPIResponse() {
        const response = await this._rankingPositionResponsePromise;
        console.log(`✓ Ranking Position filter API matched URL: ${response.url()}`);
        const body = await response.json();
        const total = body.total ?? body.totalCount ?? (Array.isArray(body) ? body.length : (body.items?.length ?? 0));
        console.log(`✓ Ranking Position filter API total: ${total}`);
        console.log('── Ranking Position Filter API Response JSON ────────────────────');
    ...
    async verifyAPITotalGreaterThanZero(responseBody) {
        ...
        const total = body.total ?? body.totalCount ?? (Array.isArray(body) ? body.length : (body.items?.length ?? 0));
        if (total > 0) {
            console.log(`✓ Ranking Position filter API total count (${total}) is greater than zero`);
        } else {
...
module.exports = CompetitionRankingPositionBasicFilterPage;
