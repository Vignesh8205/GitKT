...
class OrganizationAdvancedFilterPage {
    /**
     * @param {Page} page
     */
    constructor(page) {
    ...
    async clickOrganizationTab() {
        const tab = this.page.locator('span.e-list-text').filter({ hasText: /^Organization$/ }).first();
    ...
    async verifyOrganizationPageTitle() {
        await this.page.waitForLoadState('networkidle', { timeout: 15000 }).catch(() => {});
        const title = this.page.locator(
            "//div[normalize-space(text())='Organization'] | " +
            "//h1[normalize-space(text())='Organization'] | " +
            "//h2[normalize-space(text())='Organization'] | " +
            "//span[normalize-space(text())='Organization'] | " +
            "//span[contains(@class,'text-') and normalize-space()='Organization']"
        );
        await title.first().waitFor({ state: 'visible', timeout: 20000 });
        console.log('✓ Organization page title verified');
    }
    ...
    startCapturingAPIResponse() {
        this._apiResponsePromise = this.page.waitForResponse(
            (r) =>
                r.url().toLowerCase().includes('organization') &&
                r.request().method() === 'POST' &&
                r.status() === 200,
            { timeout: 90000 }
        );
        console.log('✓ Listening for Organization API response');
    }
    ...
    async awaitAPIResponse() {
        ...
        console.log(`✓ Captured Organization API URL: ${response.url()}`);
    ...
    async verifyAPITotalGreaterThanZero(apiBody) {
        ...
        if (count === 0 || items.length === 0) {
            console.log('ℹ No records found in Organization for the applied filter — grid displays "No records found".');
        } else {
            console.log(`✓ Organization API total count is ${count}, items in page: ${items.length}`);
        }
    }
    ...
    printAPIResponseJSON(apiBody) {
        console.log('──────── ORGANIZATION ADVANCED FILTER API RESPONSE (JSON) ────────');
    ...
    checkCondition(item, apiField, operator, value) {
        ...
        // If the field resolves to an array, check if ANY element satisfies the condition
...
module.exports = OrganizationAdvancedFilterPage;
