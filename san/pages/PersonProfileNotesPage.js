...
class PersonProfileNotesPage {
    constructor(page) {
        this.page = page;
        this.searchBox = page.locator("//span[contains(@style, 'search-icon.svg')]");
        this.quickSearchInput = page.locator("//input[contains(@placeholder, 'Search by Person Name or Code')]");
        ...
        this.templateDropdownSearch = page.locator("//input[contains(@placeholder,'Search Template')]");
    }

    async openPersonDetailsByCode(personCode) {
        await this.searchBox.waitFor({ state: 'visible', timeout: 10000 });
        await this.searchBox.click();
        await this.quickSearchInput.fill(personCode);
        ...
        const personLink = this.page.getByText(personCode, {exact: true }).first();
    ...
    async enterProfileNoteContent(noteContent) {
        const trimmedContent = noteContent.trim();
        const editorFrame = this.page.locator('[contenteditable="true"][role="textbox"]');
        try {
            await editorFrame.waitFor({ state: 'visible', timeout: 3000 });
    ...
    async enablePrivateNoteSwitch() {
        ...
        // Fallback: if only one switch exists, it represents public visibility.
        await this.publicSwitch.waitFor({ state: 'visible', timeout: 10000 });
    ...
    //scenarion # 2 for searching notes by name and editing the note content
    ...
    async pressEnterToSearchTemplate() {
        await this.templateDropdownSearch.press('Enter');
        await this.page.waitForTimeout(1500);
    }

    async verifyTemplateInList(templateName) {
        const templateItem = this.page.getByText(templateName, { exact: true });
...
module.exports = PersonProfileNotesPage;