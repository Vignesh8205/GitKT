...
class ClubSectionPersonFunctionAdvancedFilterPage {
    /**
     * @param {Page} page
     */
    constructor(page) {
        ...
        // Sections tab on club detail page
        this.sectionsTab = page.locator("//div[text()=' Sections ']");

        // First row in the Sections grid
        this.firstSectionRow = page.locator("//div[contains(@class,'e-gridcontent')]//table//tbody//tr[1]");
    ...
    async navigateToSectionsTab() {
        await this.sectionsTab.waitFor({ state: 'visible', timeout: 15000 });
    ...
    async clickFirstSectionRecord() {
        ...
        await this.page.waitForTimeout(1000);
        console.log('✓ Clicked first section record');
    }
    ...
    async openPersonFunctionFilter() {
        ...
        console.log('✓ Opened Section Person Function filter dialog');
    }
    ...
    startCapturingAPI() {
        ...
        console.log('✓ Listening for Section Person Function advanced filter API response');
    }
    ...
    async validateApiResponseCondition(responseBody, fieldLabel, operator, value) {
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            if (actual === null || actual === undefined) {
                console.warn(`⚠ Field "${apiField}" is null/undefined in Item[${i}]. Top-level keys: [${Object.keys(items[i] || {}).join(', ')}]. Skipping — update mapFieldLabel() if the key is wrong.`);
                continue;
            }
...
module.exports = ClubSectionPersonFunctionAdvancedFilterPage;
