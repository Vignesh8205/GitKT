@smoke @regression @dateGridAdvancedFilter @advanceFilter
Feature: Date Grid Advanced Filter Functionality
    As a user
    I want to filter the Date Grid using advanced filter criteria
    So that I can find specific date grid templates using complex conditions
    ...
    Scenario Outline: Verify user can apply a single condition filter using <Field> <Operator> "<Value>"
        ...
        Then  User starts capturing the date grid list API response
        Then  User clicks the Apply Advanced Filter button
        Then  User verifies the date grid API response total count is greater than zero
        Then  User prints the date grid API response JSON
        Then  User verifies all date grid API records have "<Field>" "<Operator>" "<Value>"
    ...
    @dateGridAdvancedFilterANDConditionTest
    Scenario Outline: Verify user can apply two conditions with AND logic - <Field1> and <Field2>
        ...
        Then  User verifies the date grid API response total count is greater than zero
        Then  User prints the date grid API response JSON
        Then  User verifies all date grid API records have "<Field1>" "<Operator1>" "<Value1>"
        Then  User verifies all date grid API records have "<Field2>" "<Operator2>" "<Value2>"
    ...
    @dateGridAdvancedFilterORConditionTest
    Scenario Outline: Verify user can apply two conditions with OR logic - <Field1> or <Field2>
        ...
        Then  User verifies the date grid API response total count is greater than zero
        Then  User prints the date grid API response JSON
        Then  User verifies all date grid API records satisfy "<Field1>" "<Operator1>" "<Value1>" or "<Field2>" "<Operator2>" "<Value2>"
    ...
    @dateGridAdvancedFilterInOperatorTest
    Scenario Outline: Verify user can apply a single condition filter using <Field> In "<Value>"
        ...
        Then  User verifies the date grid API response total count is greater than zero
        Then  User prints the date grid API response JSON
        Then  User verifies all date grid API records have "<Field>" "In" "<Value>"
    ...
    @dateGridAdvancedFilterNotInOperatorTest
    Scenario Outline: Verify user can apply a single condition filter using <Field> Not in "<Value>"
        ...
        Then  User verifies the date grid API response total count is greater than zero
        Then  User prints the date grid API response JSON
        Then  User verifies all date grid API records have "<Field>" "Not in" "<Value>"
    ...
    @dateGridAdvancedFilterEqualOperatorTest
    Scenario Outline: Verify user can apply a single condition filter using <Field> <Operator> "<Value>"
        # Navigate to Date Grid
        Then  User Can able to access the Main menu in home page
        Then  User choose the competition management tab
        ...
        Examples:
            | Field         | Operator  | Value         |
            | Template Name | Equal     | DateGrid0508  |
    ...
    @dateGridAdvancedFilterComparisonOperatorTest
    ...
    @dateGridAdvancedFilterBetweenOperatorTest
    Scenario Outline: Verify user can apply a <Operator> condition filter on <Field> between "<FromValue>" and "<ToValue>"
        ...
        Then  User verifies the date grid API response total count is greater than zero
        Then  User prints the date grid API response JSON
        Then  User verifies all date grid API records have "<Field>" "<Operator>" values "<FromValue>" and "<ToValue>"
